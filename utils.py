...
def get_loader(data_name, img_size=256, batch_size=8, split='test',
    ...
    dataConfig = data_config.DataConfig().get_data_config(data_name)   #dataname关系到训练数据的路径
...
def get_loaders(args):
    ...
    datasets = {'train': training_set, 'val': val_set}#20240403修改去掉验证集
    # datasets = {'train': training_set,'test': test_set}
    # xxxx=DataLoader(datasets['train'], batch_size=args.batch_size,
    #                              shuffle=True, num_workers=args.num_workers)
    dataloaders = {x: DataLoader(datasets[x], batch_size=args.batch_size,
                                 shuffle=True, num_workers=args.num_workers)
                   for x in ['train', 'val']}#20240403修改去掉验证集
    # dataloaders = {x: DataLoader(datasets[x], batch_size=args.batch_size,
    #                              shuffle=True, num_workers=args.num_workers)
    #                for x in ['train','test']}
